...
export class AgentCoreStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: AgentCoreStackProps) {
    ...
    // The LangGraph runtime owns the ticket creation transaction. Keeping this
    // table in the same stack lets the runtime receive its name and only the
    // DynamoDB permissions it needs, rather than delegating ticket creation to
    // a mock or a second agent implementation.
    const tickets = new dynamodb.Table(this, 'ServiceDeskTickets', {
      partitionKey: { name: 'id', type: dynamodb.AttributeType.STRING },
      billingMode: dynamodb.BillingMode.PAY_PER_REQUEST,
      timeToLiveAttribute: 'expiresAt',
      removalPolicy: RemovalPolicy.DESTROY,
    });
    const serviceDeskRuntime = this.application.environments.get('ServiceDeskAgent');
    if (serviceDeskRuntime) {
      serviceDeskRuntime.runtime.addEnvironmentVariable('TICKETS_TABLE', tickets.tableName);
      tickets.grantReadWriteData(serviceDeskRuntime.runtime.role);
    }
    new CfnOutput(this, 'ServiceDeskTicketsTable', { value: tickets.tableName });
